 ...
 */
void generar_middle_files(
    char *datos_desordenados,
    size_t *cantidad_mid_files
) {
  ...
  while (1) {
    ...
    while (total_leidos < NUMS_EN_RAM) { // mientras todavia quede espacio en
                                         // ram
    ...
    if (total_leidos == 0)
      break; // no se leyo ningun bloque
    ...
    while (total_escritos < total_leidos) {
      ...
      size_t escritos =
          fwrite(buffer + offset, B_BYTES, 1, f_out); // un bloque a la vez

      total_escritos += NUMS_POR_BLOQUE; // se escribio un bloque (512
                                         // elementos)
      lecturas_escrituras++; //+1 I/O
    }
...
int seleccionar_minimo(Mid *mids, size_t cantidad_mids) {
  ...
  for (int i = 0; i < cantidad_mids; i++) {
    if (mids[i].terminado || mids[i].pos >= mids[i].usados)
      continue;
...
void merge_cant_sobre_a(
  ...
  for (int index = 0; index < cuociente; index++) {
    *cantidadSupMid = cuociente + (residuo > 0);
    ...
    snprintf(
        nombre_salida, sizeof(nombre_salida), "superior_mid_%dM_%d.bin",
        cuantosM, index
    );
  ...
  for (int i = 0; i < (cuociente + (residuo > 0)); i++) {
    ...
    snprintf(
        nombre_superior_mid, sizeof(nombre_superior_mid),
        "superior_mid_%dM_%d.bin", cuantosM, i
    );
  ...
  snprintf(
      nombre_archivo_ordenado, sizeof(nombre_archivo_ordenado),
      "orden_%dM_%d.bin", cuantosM, iteracion
  );
  ...
  while (1) {
    ...
    buffer_a_disco[elementos_escritos++] =
        sup_mids[idx].buffer[sup_mids[idx].pos++];
...
void merge_middle_files(
  ...
  if (cantidad > ARIDAD) {
    ...
    merge_cant_sobre_a(
        cantidad, cuociente, residuo, cuantosM, cantidadSupMid, iteracion
    );
    return;
  }
  ...
  while (1) {
    ...
    if (idx == -1)
      break; // todos estan vacios

    buffer_a_disco[elementos_escritos++] =
        mids[idx].buffer[mids[idx].pos++]; // copio el menor primer elemento y
                                           // muevo la posicion de su puntero
    ...
    if (mids[idx].pos >= mids[idx].usados) { // agotamos los elementos del
                                             // buffer
...
void borrar_middle_files(
  ...
  for (int i = 0; i < cantidad_sup_mids; i++) {
    ...
    snprintf(
        nombre_sup_mid, sizeof(nombre_sup_mid), "superior_mid_%dM_%d.bin",
        cuantosM, i
    );
