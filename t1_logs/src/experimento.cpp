...
int main() {
  ...
  for (int m = 4; m <= 60; m += 4) {
    for (int i = 0; i < 5; i++) {
      ...
      snprintf(
          nombreOrdenado, sizeof(nombreOrdenado), "orden_%dM_%d.bin", m, i
      );
      // printf("\nArchivo ordenado: %s\n", nombreOrdenado);
      printf("Tiempo total: %.2f segundos\n", duracion_m.count());
      printf(
          "Total I/Os (lecturas + escrituras de bloques): %zu\n",
          lecturas_escrituras
      );
      ...
      std::string filenameSorted = "SORTED_" + std::to_string(m) + "M_" +
                                   std::to_string(i) + "_" +
                                   std::to_string(192) + ".bin";
      ...
      printf("Tiempo total: %.2f segundos\n", duracion_q.count());
      std::cout << "Total I/Os (lecturas + escrituras de bloques): "
                << lecturas_escrituras << std::endl;
      std::cout << "----------------------------------------\n" << std::endl;
